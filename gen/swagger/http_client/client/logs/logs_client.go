...
func (a *Client) AddLog(params *AddLogParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*AddLogOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &AddLogReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*AddLogOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for addLog: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
