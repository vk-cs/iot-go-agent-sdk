// Code generated by go-swagger; DO NOT EDIT.

package devices

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
)

// New creates a new devices API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) ClientService {
	return &Client{transport: transport, formats: formats}
}

/*
Client for devices API
*/
type Client struct {
	transport runtime.ClientTransport
	formats   strfmt.Registry
}

// ClientOption is the option for Client methods
type ClientOption func(*runtime.ClientOperation)

// ClientService is the interface for Client methods
type ClientService interface {
	GetDeviceVersionedConfig(params *GetDeviceVersionedConfigParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetDeviceVersionedConfigOK, error)

	PatchDeviceCommandStatus(params *PatchDeviceCommandStatusParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*PatchDeviceCommandStatusOK, error)

	SetTransport(transport runtime.ClientTransport)
}

/*
  GetDeviceVersionedConfig gets device versioned config
*/
func (a *Client) GetDeviceVersionedConfig(params *GetDeviceVersionedConfigParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetDeviceVersionedConfigOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewGetDeviceVersionedConfigParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "getDeviceVersionedConfig",
		Method:             "GET",
		PathPattern:        "/devices/config/{version_id}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetDeviceVersionedConfigReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
		opt(op)
	}

	result, err := a.transport.Submit(op)
	if err != nil {
		return nil, err
	}
	success, ok := result.(*GetDeviceVersionedConfigOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getDeviceVersionedConfig: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

/*
  PatchDeviceCommandStatus patches device command status
*/
func (a *Client) PatchDeviceCommandStatus(params *PatchDeviceCommandStatusParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*PatchDeviceCommandStatusOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewPatchDeviceCommandStatusParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "patchDeviceCommandStatus",
		Method:             "PATCH",
		PathPattern:        "/devices/{device_id}/commands/{command_id}/status",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &PatchDeviceCommandStatusReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
		opt(op)
	}

	result, err := a.transport.Submit(op)
	if err != nil {
		return nil, err
	}
	success, ok := result.(*PatchDeviceCommandStatusOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for patchDeviceCommandStatus: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

// SetTransport changes the transport on the client
func (a *Client) SetTransport(transport runtime.ClientTransport) {
	a.transport = transport
}
